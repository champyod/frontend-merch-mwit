"use client";

import Loader from "@/components/ui/Loader";
import { useAuth } from "@/contexts/auth-context";
import { Item } from "@/types/types";
import Link from "next/link";
import { useMemo, useState } from "react";
import { useAdminCollections, useAdminProducts, useBulkUpdateProducts } from "@/hooks/useAdmin";
import { Box, Card, Heading, Text, Button, Stack, Flex, Grid, Badge, Input } from "@/components/ui/primitives";
import { calculateSalePrice } from "@/lib/logic";
import { navigateWithLocale, normalizeLocale } from "@/lib/navigation";
import { useIntlayer, useLocale } from "next-intlayer";
import { useRouter } from "next/navigation";
import { toast } from "sonner";

const PAGE_SIZE = 12;
type ProductSort = "latest" | "title_asc" | "price_asc" | "price_desc";

export default function ProductsPage() {
	const t = useIntlayer("admin");
	const localeData = useLocale();
	const locale = normalizeLocale(localeData);
	const router = useRouter();
	const { user, isLoading: isAuthLoading } = useAuth();
	const [collectionName, setCollectionName] = useState("");
	const [searchTerm, setSearchTerm] = useState("");
	const [sortBy, setSortBy] = useState<ProductSort>("latest");
	const [currentPage, setCurrentPage] = useState(1);
	const [selectedProductIds, setSelectedProductIds] = useState<number[]>([]);

	const { data: products = [] } = useAdminProducts(collectionName, !!user);
	const { data: collections = [] } = useAdminCollections(!!user);
	const bulkUpdateProducts = useBulkUpdateProducts();

	const filteredAndSortedProducts = useMemo(() => {
		const normalizedSearch = searchTerm.trim().toLowerCase();
		const filtered = normalizedSearch.length === 0
			? products
			: products.filter((product) =>
				product.title.toLowerCase().includes(normalizedSearch) ||
				(product.name || "").toLowerCase().includes(normalizedSearch),
			);

		const sorted = [...filtered];
		switch (sortBy) {
			case "title_asc":
				sorted.sort((left, right) => left.title.localeCompare(right.title));
				break;
			case "price_asc":
				sorted.sort((left, right) => left.price - right.price);
				break;
			case "price_desc":
				sorted.sort((left, right) => right.price - left.price);
				break;
			case "latest":
			default:
				sorted.sort((left, right) => right.id - left.id);
				break;
		}

		return sorted;
	}, [products, searchTerm, sortBy]);

	const totalPages = Math.max(1, Math.ceil(filteredAndSortedProducts.length / PAGE_SIZE));
	const safeCurrentPage = Math.min(currentPage, totalPages);
	const paginatedProducts = useMemo(() => {
		const startIndex = (safeCurrentPage - 1) * PAGE_SIZE;
		return filteredAndSortedProducts.slice(startIndex, startIndex + PAGE_SIZE);
	}, [filteredAndSortedProducts, safeCurrentPage]);

	const toggleSelectProduct = (productId: number) => {
		setSelectedProductIds((prev) =>
			prev.includes(productId) ? prev.filter((id) => id !== productId) : [...prev, productId],
		);
	};

	const runBulkAction = async (action: "hide" | "show" | "disable" | "restore") => {
		if (selectedProductIds.length === 0) {
			toast.error("Select at least one product");
			return;
		}

		try {
			await bulkUpdateProducts.mutateAsync({ ids: selectedProductIds, action });
			toast.success("Bulk action applied");
			setSelectedProductIds([]);
		} catch (error) {
			toast.error(error instanceof Error ? error.message : "Bulk action failed");
		}
	};

	if (isAuthLoading || !user) return <Loader />;

	return (
		<Box className="pb-20 p-4 md:p-10">
			<Flex justifyContent="between" alignItems="center" className="mb-8">
				<Heading level={1} size="3xl" color="text-white">Products</Heading>
				<Button variant="primary" size="md" onClick={() => navigateWithLocale(router, locale, "/admin/products/add")}>
					+ Add Product
				</Button>
			</Flex>

			<Card variant="glass" className="p-6">
				<Stack gap={6}>
					{/* Bulk Actions */}
					<Flex justifyContent="between" alignItems="center" className="gap-3 flex-wrap">
						<Text size="sm" color="text-slate-400">
							Selected {selectedProductIds.length} item(s)
						</Text>
						<Flex gap={2} wrap="wrap">
							<Button variant="secondary" size="sm" onClick={() => runBulkAction("hide")} disabled={bulkUpdateProducts.isPending || selectedProductIds.length === 0}>Bulk Hide</Button>
							<Button variant="secondary" size="sm" onClick={() => runBulkAction("show")} disabled={bulkUpdateProducts.isPending || selectedProductIds.length === 0}>Bulk Show</Button>
							<Button variant="secondary" size="sm" onClick={() => runBulkAction("disable")} disabled={bulkUpdateProducts.isPending || selectedProductIds.length === 0}>Bulk Disable</Button>
							<Button variant="secondary" size="sm" onClick={() => runBulkAction("restore")} disabled={bulkUpdateProducts.isPending || selectedProductIds.length === 0}>Bulk Restore</Button>
						</Flex>
					</Flex>

					{/* 1-Line Filter Section */}
					<Box className="border-t border-white/5 pt-6">
						<Flex direction="col" gap={4} alignItems="end" className="w-full md:flex-row">
							{/* Filter by collection */}
							<Box className="w-full md:w-auto md:min-w-64">
								<label htmlFor="collection-select" className="text-xs font-bold text-slate-500 uppercase tracking-widest block mb-1">{t.filterByCollection.value}</label>
								<select
									id="collection-select"
									aria-label={t.filterByCollection.value}
									onChange={({ target }) => {
										setCurrentPage(1);
										setCollectionName(target.value);
									}}
									value={collectionName}
									className="bg-white/5 border border-white/10 rounded-xl p-3 text-sm text-white focus:outline-none focus:ring-2 focus:ring-[#58a076]/50 w-full appearance-none"
								>
									<option value="" className="bg-[#0a2735]">{t.allCollections.value}</option>
									{collections.map((collection) => (
										<option key={collection.id} value={collection.name} className="bg-[#0a2735]">
											{collection.name}
										</option>
									))}
								</select>
							</Box>

							{/* Sort by */}
							<Box className="w-full md:w-auto md:min-w-48">
								<label htmlFor="sort-select" className="text-xs font-bold text-slate-500 uppercase tracking-widest block mb-1">{t.sortBy.value}</label>
								<select
									id="sort-select"
									aria-label={t.sortBy.value}
									value={sortBy}
									onChange={({ target }) => {
										setCurrentPage(1);
										setSortBy(target.value as ProductSort);
									}}
									className="bg-white/5 border border-white/10 rounded-xl p-3 text-sm text-white focus:outline-none focus:ring-2 focus:ring-[#58a076]/50 w-full appearance-none"
								>
									<option value="latest" className="bg-[#0a2735]">{t.latestUpdated.value}</option>
									<option value="title_asc" className="bg-[#0a2735]">{t.titleAZ.value}</option>
									<option value="price_asc" className="bg-[#0a2735]">{t.priceLowHigh.value}</option>
									<option value="price_desc" className="bg-[#0a2735]">{t.priceHighLow.value}</option>
								</select>
							</Box>

							{/* Search - Pushed to the right on desktop */}
							<Box className="w-full md:flex-grow">
								<label htmlFor="search-input" className="text-xs font-bold text-slate-500 uppercase tracking-widest block mb-1">{t.search.value}</label>
								<Input
									id="search-input"
									type="text"
									placeholder={t.searchProductPlaceholder.value}
									value={searchTerm}
									onChange={({ target }) => {
										setCurrentPage(1);
										setSearchTerm(target.value);
									}}
									className="w-full"
								/>
							</Box>
						</Flex>
					</Box>

					{/* Product Grid */}
					{filteredAndSortedProducts.length === 0 ? (
						<Box className="py-20 text-center">
							<Text color="text-slate-500" italic>No products matched your filters.</Text>
						</Box>
					) : (
						<Stack gap={6}>
							<Grid cols={1} className="sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4" gap={6}>
								{paginatedProducts.map((product) => (
									<ItemCard
										item={product}
										key={product.id}
										locale={locale}
										isSelected={selectedProductIds.includes(product.id)}
										onToggleSelect={toggleSelectProduct}
									/>
								))}
							</Grid>
							
							{/* Pagination */}
							<Flex justifyContent="between" alignItems="center" className="pt-6 border-t border-white/5">
								<Text size="sm" color="text-slate-400">
									Page {safeCurrentPage} of {totalPages}
								</Text>
								<Flex gap={2}>
									<Button
										variant="secondary"
										size="sm"
										disabled={safeCurrentPage <= 1}
										onClick={() => {
											setCurrentPage((prev) => Math.max(1, prev - 1));
											window.scrollTo({ top: 0, behavior: 'smooth' });
										}}
									>
										Previous
									</Button>
									<Button
										variant="secondary"
										size="sm"
										disabled={safeCurrentPage >= totalPages}
										onClick={() => {
											setCurrentPage((prev) => Math.min(totalPages, prev + 1));
											window.scrollTo({ top: 0, behavior: 'smooth' });
										}}
									>
										Next
									</Button>
								</Flex>
							</Flex>
						</Stack>
					)}
				</Stack>
			</Card>
		</Box>
	);
}

function ItemCard({
	item,
	locale,
	isSelected,
	onToggleSelect,
}: {
	item: Item;
	locale: "th" | "en";
	isSelected: boolean;
	onToggleSelect: (productId: number) => void;
}) {
	const {
		id,
		title,
		price,
		url,
		name,
		hidden,
		last_edited_by_name,
		text,
	} = item;
	const href = `/${locale}/admin/products/edit/${id}`;

	const salePrice = calculateSalePrice(item);

	return (
		<div className="group relative">
			<input
				type="checkbox"
				title="Select product"
				checked={isSelected}
				onChange={() => onToggleSelect(id)}
				className="absolute top-3 left-3 z-10 w-4 h-4 rounded border-white/30 bg-[#0a2735]/70 cursor-pointer"
			/>
			<Link href={href} className="block">
				<Card 
					variant="outline" 
					className={`p-4 h-full transition-all hover:border-[#58a076]/50 hover:bg-white/5 ${hidden === 1 ? "opacity-50" : ""}`}
				>
					<Stack gap={4}>
						<Box className="relative aspect-square rounded-2xl overflow-hidden bg-white/5 border border-white/5">
							<img
								className="object-cover w-full h-full transition-transform group-hover:scale-110"
								src={url || "/images/logo.png"}
								alt={title}
							/>
							{hidden === 1 && (
								<Box className="absolute inset-0 flex items-center justify-center bg-[#0a2735]/60">
									<Badge variant="default">Hidden</Badge>
								</Box>
							)}
						</Box>

						<Stack gap={1}>
							<Flex justifyContent="between" wrap="wrap" gap={1}>
								<Badge variant="info">{name}</Badge>
								{text && <Badge variant="success">{text}</Badge>}
							</Flex>
							<Text weight="bold" color="text-white" className="truncate block mt-2">{title}</Text>
							
							<Box className="mt-1">
								{salePrice > 0 ? (
									<Flex gap={2} alignItems="baseline">
										<Text weight="black" color="text-[#58a076]" size="lg">฿{salePrice.toLocaleString()}</Text>
										<Text size="xs" color="text-slate-500" className="line-through">฿{price.toLocaleString()}</Text>
									</Flex>
								) : (
									<Text weight="black" color="text-white" size="lg">฿{price.toLocaleString()}</Text>
								)}
							</Box>
						</Stack>

						<Box className="pt-2 border-t border-white/5">
							<Text size="xs" color="text-slate-500">Edited by: {last_edited_by_name}</Text>
						</Box>
					</Stack>
				</Card>
			</Link>
		</div>
	);
}
